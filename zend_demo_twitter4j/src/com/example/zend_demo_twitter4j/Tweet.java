...
public class Tweet extends Activity implements OnClickListener {
	...
	/**
	 * Method called whenever this Activity starts - get ready to tweet Sets up
	 * twitter and onClick listeners - also sets up for replies
	 */
	private void setupTweet() {
		// prepare to tweet
		// get preferences for user twitter details
		tweetPrefs = getSharedPreferences("prefs_twitter", 0);
		...
		case R.id.dotweet:
			...
			try {
				...
				if (tweetName.length() > 0)
					tweetTwitter.updateStatus(new StatusUpdate(toTweet)
